...
class Game extends BaseController
{
    ...
    {
        $info = [];
        $this->dealIndexInfo($info); //分开 每次模板不好展示
        return view("/Statics/Pc/game/game.html", $info);
    }
    /**
     * 应用列表
     */
    ...
    protected function dealIndexInfo(&$info)
    {
        ...
        $gameZtList = array_chunk($gameZtList, 3);
     ...
     * @return void
     */
    protected function dealSoftInfo(&$info)
    {
        $cateService = new CategoryService();
    ...
    protected function dealGameDetail($union_id, &$info)
    {
        ...
        //好游安利
        $goodGameList = $adService->getAdvGameOrAppPackList(PcEnum::PC_GAME_DETAIL_GOOD);
    ...
    protected function dealSoftDetail($union_id, &$info)
    {
        ...
        //好游安利
        $goodGameList = $adService->getAdvGameOrAppPackList(PcEnum::PC_APP_DETAIL_GOOD);

        $downGameList = $gameService->getAppList($gfield,5,'wview desc,uptime desc');
    ...
    public function gameDownload($union_id = '')
    {
        ...
        return view("/Statics/Pc/game/download.html", $info);
    }
