...
class Crontab extends Controller
{
    ...
    public function statisticsNewsView()
    {
        ...
        foreach ($keyData as $key => $val) {
            $redisKey = $this->webId.':news:view:detail:' . $time . ':' . $val;
    ...
    public function statisticsGameView()
    {
        ...
        foreach ($keyData as $key => $val) {
            $redisKey = $this->webId.':game:view:detail:' . $time . ':' . $val;
    ...
    public function statisticsAppView()
    {
        ...
        foreach ($keyData as $key => $val) {
            $redisKey = $this->webId.':app:view:detail:' . $time . ':' . $val;
    ...
    public function insertStatisticsView($redisKey, $field, $type = 1)
    {
        ...
        echo " this key is:" . $redisKey . PHP_EOL;
        $stime = explode(' ', microtime());
        $stimes = $stime[1] + $stime[0];
        $list = $redis->getByHashKey($redisKey);
        $arr = array();
        if ($list) {
            //统计相同id的和
     ...
     * 条件：专题数量为0
     */
    public function updateAllZeroAppTagsGameNums()
    {
        //var_dump($this->request->getOption('option1')); //高版本获取cli参数
        //$this->input->getOption('option1'); 低版本
        set_time_limit(0);
        echo 'start-' . date('Y-m-d H:i:s', time()) . PHP_EOL;
        $tagModel = new TagModel();
        $tagwhere="status = 1 and game_num = 0 and type in (1,2)";
        $list = $tagModel->getTableList($tagwhere, 'id,type');
        if (count($list)<=0){
            echo '-' . date('Y-m-d H:i:s', time()) . '-未找到game_num为0的专题' . PHP_EOL;
            die;
        }
        $gameList = new GameListModel();
        $appList = new AppListModel();
        $updateData = array();
        $updateGameCount = $updateAppCount = 0; //更新的条数
        if (!empty($list)) {
            foreach ($list as $key => $val) {
                if ($val['type'] == 1) {
                    $gwhere = " classify = 1 and status = 1 and state = 1 and find_in_set({$val['id']},tags)";
                    $total = $gameList->getTableListCount($gwhere);
                    if ($total > 0) {
                        $updateData[$key]['game_num'] = $total;
                        $updateData[$key]['game_num_time'] = time();
                        $updateData[$key]['id'] = $val['id'];
                        $updateGameCount++;
                    }
                } else {
                    $gwhere = " classify = 2 and status = 1 and state = 1 and find_in_set({$val['id']},tags)";
                    $total = $appList->getTableListCount($gwhere);
                    if ($total > 0) {
                        $updateData[$key]['game_num'] = $total;
                        $updateData[$key]['game_num_time'] = time();
            ...
            echo '-' . date('Y-m-d H:i:s', time()) . '-查找到游戏数量' . $updateGameCount . PHP_EOL;
            echo '-' . date('Y-m-d H:i:s', time()) . '-查找到应用数量' . $updateAppCount . PHP_EOL;
            $tagModel = new TagModel();
            if ($updateData){
                //可分批
                $flag =$tagModel->upTagsBatch($updateData);
                if ($flag > 0) {
                    echo '-' . date('Y-m-d H:i:s', time()) . '-更新完成' . PHP_EOL;
                } else {
                    echo '-' . date('Y-m-d H:i:s', time()) . '-更新失败' . PHP_EOL;
                }
            }else{
                echo '-' . date('Y-m-d H:i:s', time()) . '-数据为空更新失败' . PHP_EOL;
            }
        } else {
            echo '-' . date('Y-m-d H:i:s', time()) . '-数据为空更新失败' . PHP_EOL;
        }
