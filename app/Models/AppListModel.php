...
class AppListModel extends BaseModel
{
    protected $table = 'app_list';

    public function getAppList($where = array(), $field = '*', $limit = 0, $offset = 0, $order = '', $group = '')
    {
     ...
     */
    public function getNewAppList($where = [], $whereIn = [], $field = '*', $limit = 0, $offset = 0, $order = '', $group = '')
    {

        $query = $this->db->table('app_list as bgl')
            ->join('app_pack as bgp', 'bgl.id=bgp.gameid', 'left')
            ->where($where);
     ...
     */
    public function appNewCount($where = array())
    {

        return $this->db->table('app_list as bgl')
            ->join('app_pack as bgp', 'bgl.id=bgp.gameid')
            ->where($where)
            ->select('COUNT(*) AS num')
            ->countAllResults();
    }
    ...
    public function getGameInfoById ($id, $getShortImg = false, $getGamePack = false, $getTypeName = false)
    {
        ...
        $gamePackModel = new AppPackModel();
        $gameImgModel = new AppImgModel();
        ...
        $game_pack = '';
        if ($getGamePack) {
            $game_pack = $gamePackModel->getOneInfoByWhere(['gameid' => $id], 'id,gameid,and_url,and_size,and_ver,and_unit,ios_ver,ios_size,ios_url,ios_unit,pc_url,and_md5,and_pkgname');
            if ($game_pack) {
        ...
        $typeHref = '';
        if ($getTypeName) {
            // $typeInfo = $CategoryModel->getInfoById($gameInfo['type']);
        ...
        //月下载次数hardcode
        $gameInfo['mdown'] = rand(999, 100000);
        $gameInfo['now_play'] = rand(3000, 10000); //在玩人数
        $gameInfo['now_like'] = rand($gameInfo['now_play'] - 1000, $gameInfo['now_play']); //好玩人数
    ...
    public function setAppView($data, $field){
    ...
    public function getMasterGameList(string $where = "", $field = ['*'], $limit = 0, $offset = 0, $order = '', $group = 'bg.id')
    {
        return $this->db->table('app as bg')
            ...
            ->join('similar_app as sg', 'bg.id = sg.fid')
            ->select($field.',count(sg.fid) as snums')
            ->where("bg.name = bgl.name and bg.status = 1 and bg.state = 1 and bg.show_down = 1 and sg.classify = 2 ")
            ->where($where?:[])
            ->groupBy($group)
            ->having('snums >= 1')
            ->orderBy($order)
    ...
    {
        $query = $this->db->table('app_tag as t')
            ->join('app_list as bgl','t.gameid=bgl.id','left')
            ->join('app_pack as bgp', 'bgl.id=bgp.gameid', 'left')
            ->where($where);
    ...
    {
        return $this->db->table('app_tag as t')
            ->join('app_list as bgl','t.gameid=bgl.id','left')
            ->where($where)
            ->countAllResults();
    }
